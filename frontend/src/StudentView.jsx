...
export default function StudentView() {
  ...
  const [enrollmentStatus, setEnrollmentStatus] = useState("pending"); // pending | complete
  const [suspiciousFlags, setSuspiciousFlags] = useState([]);
  const [keystrokeCount, setKeystrokeCount] = useState(0);
  const [gazeStatus, setGazeStatus] = useState("initializing"); // initializing | active | off-screen
  const [gazeAlerts, setGazeAlerts] = useState([]);
  ...
  useEffect(() => {
    ...
    init().catch((err) => {
      ...
      const detail = err.response
        ...
        : err.message;
      setInitError(
        `Could not connect to backend (${detail}). Make sure uvicorn is running on port 8000.`,
      );
    });
  }, []);
  ...
  const handleEditorMount = useCallback(
    (editor) => {
      ...
      const capture = new KeystrokeCapture(
        ...
        (scoreData) => {
          ...
          if (scoreData.score !== undefined) {
            setConfidenceScore(scoreData.score);
          }
      ...
      const gazeTracker = new GazeTracker(
        ...
        (gazeData) => {
          // Update gaze status
          setGazeStatus(gazeData.onScreen ? "active" : "off-screen");
        },
        (gazeAlert) => {
          // Log gaze alerts
          setGazeAlerts((prev) => [
      ...
      // Paste detection
      const pasteDetector = new PasteDetector(sessionId, (pasteEvent) => {
        setPasteEvents((prev) => [
          ...prev.slice(-9),
          {
            ...pasteEvent,
      ...
      pasteDetector.attach(container);
  ...
  useEffect(() => {
    ...
    snapshotIntervalRef.current = setInterval(() => {
      axios
        .post("/api/snapshot", { session_id: sessionId, code })
        .catch(() => {});
    }, 60000);
  ...
  // Cleanup trackers when leaving coding stage
  useEffect(() => {
    if (stage !== STAGE_CODING) {
      if (gazeTrackerRef.current) {
        gazeTrackerRef.current.stop();
      }
      if (pasteDetectorRef.current) {
        pasteDetectorRef.current.detach();
      }
      ...
      <div style={styles.done}>
        ...
        {submitReport && (
          <div style={styles.scoreBox}>
            ...
            <p>
              <strong>Total: {submitReport.scores?.total_score}%</strong>
            </p>
          </div>
        )}
        <p style={{ color: "#888", marginTop: 16 }}>
          <a href="/instructor" style={{ color: "#4fc3f7" }}>
            → Instructor view
          </a>
        </p>
      </div>
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.header}>
          ...
          <>
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              {enrollmentStatus === "pending" && (
                <div
                  style={{ fontSize: 12, color: "#ff9800", fontWeight: 600 }}
                >
                  Enrollment: {keystrokeCount}/100 keystrokes
                </div>
              )}
              ...
              {suspiciousFlags.length > 0 && (
                <div
                  style={{ fontSize: 12, color: "#f04040", fontWeight: 600 }}
                >
                  ⚠ {suspiciousFlags.length} suspicious event(s)
                </div>
              )}
              {gazeStatus && (
                <div
                  style={{
                    fontSize: 12,
                    color: gazeStatus === "active" ? "#4caf50" : "#f04040",
                    fontWeight: 600,
                  }}
                >
                  👁️ {gazeStatus === "active" ? "Active" : "Off-screen"}
                </div>
              )}
              {gazeAlerts.length > 0 && (
                <div
                  style={{ fontSize: 12, color: "#ff5722", fontWeight: 600 }}
                >
                  👁️ {gazeAlerts.length} alerts
                </div>
              )}
              {pasteEvents.length > 0 && (
                <div
                  style={{ fontSize: 12, color: "#ff5722", fontWeight: 600 }}
                >
                  📋 {pasteEvents.length} paste(s) detected
                </div>
              )}
              {confidenceScore != null && (
                <div
        ...
        {stage === STAGE_CODING && (
          <a href="/instructor" style={styles.headerLink}>
            Instructor view →
          </a>
        )}
      </div>
        ...
        <div style={styles.journalGate}>
          <div style={styles.journalPanel}>
            ...
            <p style={{ color: "#aaa", fontSize: 13 }}>
              Before you start coding, describe your approach. Minimum 100
              characters.
            </p>
            <textarea
            ...
            <div style={styles.journalFooter}>
              <span
                style={{
                  color: journalText.length >= 100 ? "#4caf50" : "#888",
                }}
              >
              ...
              <button
                style={{
                  ...styles.btnPrimary,
                  opacity: journalText.length < 100 ? 0.5 : 1,
                }}
              ...
              <div style={styles.testResults}>
                {testResults.map((r) => (
                  <div key={r.test_name} style={styles.testRow}>
                    ...
                    {!r.passed && r.stderr && (
                      <span
                        style={{ color: "#888", marginLeft: 8, fontSize: 12 }}
                      >
                        {r.stderr.slice(0, 80)}
                      </span>
                    )}
                    {!r.passed && r.stdout && (
                      <span
                        style={{ color: "#ccc", marginLeft: 8, fontSize: 12 }}
                      >
                        {r.stdout.slice(0, 80)}
                      </span>
                    )}
