...
export default function InstructorView() {
  ...
  useEffect(() => {
    const load = () =>
      axios.get("/api/instructor/cohort").then((r) => setCohort(r.data));
    load();
  ...
  const handleRowClick = async (sessionId) => {
    ...
    try {
      const res = await axios.get(
        `/api/instructor/session/${sessionId}/timeline`,
      );
  ...
  const handleSaveCompScore = async () => {
    ...
    await axios.patch(
      `/api/instructor/session/${selectedId}/comprehension-score`,
      {
    ...
    setCohort((c) =>
      c.map((s) =>
        s.session_id === selectedId
          ? { ...s, comprehension_score: parseFloat(compScore) }
          : s,
      ),
    );
  };
  ...
  return (
    <div style={styles.root}>
      <div style={styles.header}>
        ...
        <a href="/student" style={styles.headerLink}>
          ← Student view
        </a>
      </div>
              ...
              {sorted.map((s) => (
                ...
                >
                  ...
                  <td
                    style={{
                      ...styles.td,
                      color: s.anomaly_count >= 3 ? "#ffa500" : "#e6edf3",
                    }}
                  >
                    {s.anomaly_count}
                  </td>
                  <td
                    style={{
                      ...styles.td,
                      color: s.max_diff_lines >= 50 ? "#ffa500" : "#e6edf3",
                    }}
                  >
                    {s.max_diff_lines}
                  </td>
          ...
          <div style={styles.timeline}>
            ...
            {timeline && (
              <>
                ...
                <div style={styles.sectionHeader}>Anomalies detected</div>
                <div style={styles.anomalySection}>
                  {/* Keystroke biometric anomalies */}
                  {timeline.keystroke_windows &&
                    timeline.keystroke_windows.filter(
                      (w) => w.similarity_score < 0.5,
                    ).length > 0 && (
                      <div style={styles.anomalyGroup}>
                        <div style={styles.anomalyGroupTitle}>
                          🔐 Keystroke Biometric Anomalies (
                          {
                            timeline.keystroke_windows.filter(
                              (w) => w.similarity_score < 0.5,
                        ...
                        <div style={styles.anomalyList}>
                          {timeline.keystroke_windows
                            .filter((w) => w.similarity_score < 0.5)
                            .map((w) => (
                              <div key={w.id} style={styles.anomalyItem}>
                                ...
                                >
                                  Similarity: {w.similarity_score.toFixed(2)}
                                </span>
                              </div>
                            ))}
                        </div>
                  ...
                  {/* Gaze anomalies */}
                  {timeline.gaze_events &&
                    timeline.gaze_events.filter(
                      (g) => g.event_type === "off-screen",
                    ).length > 0 && (
                      <div style={styles.anomalyGroup}>
                        <div style={styles.anomalyGroupTitle}>
                          👁️ Gaze Anomalies (
                        ...
                        <div style={styles.anomalyList}>
                          {timeline.gaze_events
                            .filter((g) => g.event_type === "off-screen")
                            .map((g) => (
                              <div key={g.id} style={styles.anomalyItem}>
                                ...
                                <span style={styles.anomalyDuration}>
                                  Duration:{" "}
                                  {g.event_data?.duration
                                    ? Math.floor(g.event_data.duration / 1000) +
                                      "s"
                                    : "—"}
                                </span>
                              </div>
                  ...
                  {/* Paste detection */}
                  {timeline.paste_events &&
                    timeline.paste_events.length > 0 && (
                      <div style={styles.anomalyGroup}>
                        <div style={styles.anomalyGroupTitle}>
                          📋 Paste Events ({timeline.paste_events.length})
                        </div>
                        <div style={styles.anomalyList}>
                          {timeline.paste_events.map((p) => (
                            <div key={p.id} style={styles.anomalyItem}>
                              ...
                              <span style={styles.anomalyData}>
                                {p.paste_length} chars via {p.paste_source}
                              </span>
                              {p.paste_content_preview && (
                                <div style={styles.pastePreview}>
                                  Preview: "
                                  {p.paste_content_preview.substring(0, 50)}
                                  {p.paste_content_preview.length > 50
                                    ? "..."
                                    : ""}
                                  "
                                </div>
                  ...
                  {(!timeline.keystroke_windows ||
                    ...
                    (!timeline.paste_events ||
                      timeline.paste_events.length === 0) && (
                      <div style={{ color: "#8b949e", padding: "16px" }}>
                ...
                <div style={styles.comprehensionSection}>
                  {timeline.comprehension_answers ? (
                    Object.entries(timeline.comprehension_answers).map(
                      ([idx, answer]) => (
                        <div key={idx} style={{ marginBottom: 20 }}>
                          <div style={styles.questionText}>
                            Q{parseInt(idx) + 1}:{" "}
                            {timeline.session.problem_id === "stack_001"
                              ? STACK_QUESTIONS[parseInt(idx)]
                    ...
                    <div style={{ color: "#888" }}>
                      No comprehension answers submitted.
                    </div>
                  )}
